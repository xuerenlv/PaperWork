...
from utils_no_crwal import shuffle_queue, uid_is_numbers
...
class crawl_set_time_with_keyword( threading.Thread ):
    ...
    def __init__( self, uid, url_queue, thread_name = 'crawl_one_users_weibo' ):
        threading.Thread.__init__( self )
        self.name = thread_name
        self.uid = uid
...
#         
#         url = ""
#         for one_uid in self.uid_list:
#             if uid_is_numbers( one_uid ):
#                 url = "http://weibo.cn/u/" + one_uid + "?page=1"
#             else:
#                 url = "http://weibo.cn/" + one_uid + "?page=1"
#             self.url_map_uid[url] = one_uid
...
#         int_total_num = int( total_num[ total_num.find( '[' ) + 1:-1] )
#         
#         total_page = 0;
...
#                 self.url_map_uid[url] = self.url_map_uid[first_page_url] 
#                 self.url_queue.put( url )
#         pass
...
#                 self.put_second_and_more_url_queue( total_num, url )
    ...
    # 第二个版本，将微博存储到数据库中 moreinfo
    def store_weibo_to_db( self, uid, weibo_list ):
        global Single_weibo_with_more_info_store
        for weibo in weibo_list:
            with switch_collection( Single_weibo_with_more_info_store, "retweet_id_" + uid ) as Single_weibo_with_more_info_store:
                unique_single = Single_weibo_with_more_info_store( uid , content = weibo.content, praise_num = weibo.praise_num,
    ...
    def run( self ):
        shuffle_queue( self.url_queue )
        while not self.url_queue.empty():
            url = self.url_queue.get()
            weibo_list = self.crawl( url )
            self.store_weibo_to_db( self.uid, weibo_list )
        pass
...
def page_parser_from_search_with_more_info( page ):
    ...
    all_weibo_num = ''
    for one_div_u in out_soup.findAll( 'div', attrs = {'class':'u'} ):
        one_span_tc = one_div_u.find( 'span', attrs = {'class':'tc'} )
        if one_span_tc is not None:
            all_weibo_num = one_span_tc.getText()
...
#             print come_from_nickname,come_from_url
            ...
            singleWeibo_with_more_info_object = SingleWeibo_with_more_info( content_text, praise_num,
