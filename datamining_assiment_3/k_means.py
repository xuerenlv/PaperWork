...
def minmize_objective_function(file_name, loop_time, k):
    ...
    for i in range(loop_time):
        this_dist = 0.0
        k_central, central_map_example = k_means(file_list, k)
        for j in range(k):
...
#*****************         评价标准          **********************************
# purity
def gen_purity(file_list, lable_list, result_central_map_example, cluster_num):
    # 初始化 m（i，j）二维数组
...
# Gini
def gen_gini(file_list, lable_list, result_central_map_example, cluster_num):
    # 初始化 m（i，j）二维数组
    gen_matrix = [[0 for j in range(cluster_num)] for i in range(cluster_num)]
    ...
    for j in range(cluster_num):
        ...
        for i in range(cluster_num):
            g_j[j] += (float(gen_matrix[i][j]) / float(M_j)) ** 2
        g_j[j] = 1 - g_j[j]
...
if __name__ == '__main__':
    ...
    print obj_func
    purity = gen_purity(file_list, lable_list, result_central_map_example, 2)
    gini = gen_gini(file_list, lable_list, result_central_map_example, 2)
    print purity, gini
    
    
    file_list, lable_list, result_k_central, result_central_map_example, obj_func = minmize_objective_function("mnist.txt", 10, 10)
    print obj_func
    purity = gen_purity(file_list, lable_list, result_central_map_example, 10)
    gini = gen_gini(file_list, lable_list, result_central_map_example, 10)
    print purity, gini
    pass
