...
#     http://weibo.cn/breakingnews?f=search_0
    def init_url_queue( self ):
        for uid_or_nickname in self.uid_or_uname_list:
            ...
            if len( UserInfo_store.objects( uid_or_uname = str( uid_or_nickname ) ) ) != 0 or len( UserInfo_store.objects( nickname = str( uid_or_nickname ) ) ) != 0:
                continue
#             
#             # # 对这三种不同的方式作处理
...
#             self.url_queue.put( url )
    ...
    def crawl( self, uid_or_nickname, is_again = False ):
        ...
        url = ''
        if len( UserInfo_store.objects( uid_or_uname = str( uid_or_nickname ) ) ) != 0 or len( UserInfo_store.objects( nickname = str( uid_or_nickname ) ) ) != 0 \
            or len( Bie_Ming_store.objects( uid_or_uname = str( uid_or_nickname ) ) ) != 0 or len( Bie_Ming_store.objects( bie_ming = str( uid_or_nickname ) ) ) != 0 :
...
#             print user_info.to_string()
            if not  user_info == "nothing" :
                self.store_userinfo_to_db( url, user_info )
        pass  
