...
from store_model import Single_weibo_store, UserInfo, UserInfo_store,\
    UserInfo_loc, UserInfo_loc_store
...
#     http://weibo.cn/breakingnews?f=search_0
    def init_url_queue(self):
        for uid_or_nickname in self.uid_or_uname_list:
            ...
            ## 对这三种不同的方式作处理
            # http://weibo.cn/chobitsr
            # http://weibo.cn/1564320652
            # http://weibo.cn/n/%E8%8B%B1%E5%8B%87XIAO%E6%9C%88
            ##
            url = ""
...
#                 loginer.del_proxy()
#             WeiboSearchLog().get_scheduler_logger().warning(self.name+" proxy exception , change proxy !")
#             WeiboSearchLog().get_scheduler_logger().warning(url + "   parser exception")
#             crawl_set_time_with_keyword.del_proxy_lock.release()
            if is_again:
                return self.crawl(url,is_again=False)
            else:
...
#     fensi = StringField()
    ...
    def store_userinfo_to_db(self,user_info):
        if type(user_info) is str:
            WeiboSearchLog().get_scheduler_logger().info(self.name+" nothing !")
            return
        ...
        try:
            ...
            WeiboSearchLog().get_scheduler_logger().info(self.name+" insert to database, not unique !")
            return
        except:
    ...
    def run(self):
        ...
        while not self.url_queue.empty() or not self.second_url_queue.empty():
            ...
            if not self.url_queue.empty():
                ...
                url = self.second_url_queue.get()
            user_info = self.crawl(url)
#             print user_info.to_string()
            self.store_userinfo_to_db(user_info)
        pass  
