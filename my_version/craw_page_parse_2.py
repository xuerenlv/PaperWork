...
def page_parser_from_search_for_UserInfo(page,url):
    ...
    div_u_first = out_soup.find('div', attrs={'class':'u'})
    ...
    for span_class_ctt_one in span_class_ctt:
        span_class_ctt_one_html = str(span_class_ctt_one) 
        
        if u"加关注" in span_class_ctt_one_html:
#             print span_class_ctt_one_html
...
#                 nickname_candidate = nickname_candidate[:nickname_candidate.find(u'女')-1] 
    ...
    user_info = UserInfo(uid_or_uname,nickname,is_persion,check_or_not,fensi)
    return user_info
    pass
